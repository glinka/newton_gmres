...
def arnoldi_iter(A, V_init, kmax, H_init=None, f_init=None, zero_tol=1e-14):
    ...
    for k in range(k_init, kmax):
        ...
        for i in range(k+1):
            H[i, k] = np.dot(V[:, i], f)
            f = f - H[i, k]*V[:, i]
        # should test for orthogonality, and whether v_norm > 0
    return [V, H, f]
    ...
    while err > tol and iters < qr_maxiter:
        ...
        for i in range(n-1):
            if np.abs(A[i, i+1]) < zero_tol:
                ...
                return [S, thetas]
            # error is measured by Gershgorin's
...
def implicitly_restarted_arnoldi_symmetric(A, v, k, p, tol=1e-14, iram_maxiter=10, **kwargs):
    ...
    while err > tol and iters < iram_maxiter:
        ...
        S, thetas = qr_explicitly_shifted_tridiag(H, **kwargs)
...
if __name__=="__main__":
    ...
    p = 8
    # kwargs = {'maxiter': 25}
    S, thetas, V = implicitly_restarted_arnoldi_symmetric(A, v, k, p, iram_maxiter=25, qr_maxiter=25)
    eigs = np.linalg.eigvalsh(A)
    si = np.argsort(np.abs(eigs))
    print eigs[si[-k:]] - thetas
    print thetas
    print A.shape, V.shape, S.shape
    print np.dot(A, np.dot(V, S[:,-1])) - np.dot(V, S[:,-1])*thetas[-1]
